...
- Start each new feature cycle with a co-design conversation (goal, constraints, architecture options).
- Keep ACs explicit and student-readable (clear terminology, concrete outcomes).
- Separate "must implement now" scope from "optional improvements" in every backlog artifact.
